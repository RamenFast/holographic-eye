...
(async () => {
  ...
  try {
    ...
    const result = await page.evaluate(async (skipPerf) => {
      ...
      const view = {w:3200,h:1200,cx:1350,cy:375,scale:1,ui:1,selected:new Set([30]),hover:31,highlighted:new Set([32]),exclusions:[]};
      ...
      check(first.labels[0].id===30 && first.labels[1].id===31 && first.labels[2].id===32,"priority");
      ...
      labels.sync(huge);
      const perfView={...view,w:1200,h:750,cx:0,cy:0,scale:0.5};
      const times=[];
      ...
      check(field.labelStatus().accepted>0,"Field default renders content");
